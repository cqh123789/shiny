...
data <- data %>%
  mutate(draft_year = as.numeric(as.character(draft_year)),
         ...
         draft_number = as.numeric(as.character(draft_number)))
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput("draft_year", "Select Draft Year:", choices = sort(unique(data$draft_year))),
...
server <- function(input, output, session) {
  ...
  output$pieChart <- renderPlot({
    ...
    ggplot(pie_data, aes(x = "", y = num_players, fill = country)) +
      ...
      coord_polar(theta = "y") +
      theme_void() +
      labs(title = paste("Top 5 Countries in", input$draft_year, "Draft Year"), 
           fill = "Country") +
      scale_fill_manual(values = scales::hue_pal()(nrow(pie_data)),
                        labels = paste0(pie_data$country, " (", round(pie_data$percentage, 1), "%)")) +
      theme(legend.text = element_text(size = 14))  # Increase pie chart legend text size
  })
